...
SD1OPLAdaptor::SD1OPLAdaptor(SD1DeviceBase* device) : device(device), oplReg(), tones(), changes()
{
    this->initState();
}
...
void SD1OPLAdaptor::hardReset()
{
    ...
    for (uint8_t v = 0; v < 16; v++) {
        ...
        this->device->writeReg(0x11, 0x01);
...
{
    uint8_t oldData = this->oplReg.read(addr);
    this->oplReg.write(addr, data);

    uint8_t oplVoice = OPL::getVoiceForRegister(addr, this->oplReg.read(0x104));
    if (oplVoice < 24) {
...
void SD1OPLAdaptor::update()
{
    ...
    for (uint8_t v = 0; v < 16; v++) {
        ...
        if (oplVoice >= 0 && this->changes[oplVoice]) {
            OPLTone oplTone;
            this->oplReg.readTone(oplVoice, oplTone);
...
void SD1OPLAdaptor::oplGetBlockFNum(uint8_t oplVoice, uint8_t& block, uint16_t& fnum)
{
    ...
    uint8_t fnumL = this->oplReg.read(0xa0 + regOffsets[0]);
    uint16_t blockFNumH = this->oplReg.read(0xb0 + regOffsets[0]);
...
void SD1OPLAdaptor::handleOPLFNUMLChange(uint16_t addr, uint8_t oldData)
{
    ...
    uint8_t oplVoice = OPL::getVoiceForChannelRegOffset(offset, this->oplReg.read(0x104));
    uint8_t data = this->oplReg.read(addr);
    if (data != oldData) {
        this->changes[oplVoice] = true;
    }
}
